...
namespace xcbw {
    ...
    public:
        ...
        pixmap_t(const connection_t& c, uint8_t depth, drawable_t drawable, uint16_t width,
                 uint16_t height);
        pixmap_t(const connection_t& c, uint8_t depth, drawable_t drawable, point_t<uint16_t> size)
            : pixmap_t(c, depth, drawable, size.x, size.y) {}

        void free_pixmap(const connection_t& c);
