...
namespace xcbw {
    intern_atom_t::intern_atom_t(connection_t& c, bool only_if_exists, uint16_t name_len,
                                 const char* name)
        : intern_atom_t(c, intern_atom_cookie_t(
                               xcb_intern_atom(get(c), only_if_exists, name_len, name).sequence)) {}

    intern_atom_t::intern_atom_t(connection_t& c, bool only_if_exists, const char* name)
        : intern_atom_t(c, only_if_exists, std::strlen(name), name) {}

    intern_atom_t::intern_atom_t(connection_t& c, intern_atom_cookie_t cookie)
